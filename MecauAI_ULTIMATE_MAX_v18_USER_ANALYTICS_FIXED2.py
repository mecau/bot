...
def get_admin_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🖥 Сервер"), KeyboardButton(text="🛡 Админ-панель")],
            [KeyboardButton(text="👥 Пользователи"), KeyboardButton(text="📜 История")],
        ],
        resize_keyboard=True,
        is_persistent=True,
    )
...
@dp.message(F.text)
async def handle_title_page_location_year(message: Message):
    user_id=message.from_user.id
    ...
    if step=='institution':
        if len(raw)<4:
            await message.answer('⚠️ Укажи полное название колледжа/вуза.'); return
        state['institution']=raw[:500]; state['step']='student'
        await message.answer('2️⃣ <b>ФИО студента</b>\n\nНапример: <code>Иванов Иван Иванович</code>',parse_mode='HTML'); return
    if step=='student':
        if len(raw)<5:
            await message.answer('⚠️ Укажи полное ФИО студента.'); return
        state['student']=raw[:250]; state['step']='teacher'
        await message.answer('3️⃣ <b>ФИО преподавателя</b>\n\nНапример: <code>Петров Пётр Петрович</code>',parse_mode='HTML'); return
    if step=='teacher':
        if len(raw)<5:
            await message.answer('⚠️ Укажи полное ФИО преподавателя.'); return
        state['teacher']=raw[:250]; state['step']='topic'
        await message.answer('4️⃣ <b>Тема работы</b>\n\nНапиши точное название темы.',parse_mode='HTML'); return
    if step=='topic':
        if len(raw)<3:
            await message.answer('⚠️ Тема слишком короткая. Укажи полное название.'); return
        state['topic']=raw[:1000]; state['step']='location_year'
        await message.answer('5️⃣ <b>Город и год</b>\n\nНапиши в формате:\n<code>Москва, 2026</code>',parse_mode='HTML'); return
    if step=='location_year':
        m=re.match(r'^\s*(.+?)\s*[,;]\s*(\d{4})\s*$',raw) or re.match(r'^\s*(.+?)\s+(\d{4})\s*$',raw)
        if not m:
            await message.answer('⚠️ Не понял формат.\nПример: <code>Москва, 2026</code>',parse_mode='HTML'); return
        ...
        await message.answer(_title_preview_text(state),parse_mode='HTML',reply_markup=_title_preview_keyboard()); return
    if step=='confirm':
        await message.answer('ℹ️ Проверь предпросмотр выше и нажми кнопку подтверждения.'); return
...
# ========================= ADMIN MAX CONTROL CENTER =========================
# Дополнительные административные возможности. Они не вмешиваются в обычный
# пользовательский UX и доступны только владельцу/админам.

ADMIN_AUDIT_LOG = {}
ADMIN_ANNOUNCEMENT_STATE = {}
...
def _admin_log(user_id: int, action: str, details: str = ""):
    ADMIN_AUDIT_LOG.setdefault(user_id, []).append({
        "action": action,
        "details": details[:1000],
        "ts": datetime.now().isoformat(timespec="seconds"),
    })
    # Ограничиваем память audit-log.
    ADMIN_AUDIT_LOG[user_id] = ADMIN_AUDIT_LOG[user_id][-100:]

def _admin_runtime_report() -> str:
    try:
        import shutil as _shutil
        total, used, free = _shutil.disk_usage("/")
        disk = f"{used/1024**3:.1f}/{total/1024**3:.1f} GB ({free/1024**3:.1f} GB свободно)"
    except Exception:
        disk = "н/д"

    health = get_runtime_health() if "get_runtime_health" in globals() else {}
    ram = health.get("ram_percent")
    ram_free = health.get("ram_available_mb")
    busy = health.get("busy", len(busy_users) if "busy_users" in globals() else 0)
    workflows = health.get("workflows", 0)

    return (
        "🛡 <b>ADMIN — состояние системы</b>\n\n"
        f"💾 RAM: <b>{ram if ram is not None else 'н/д'}%</b>"
        f"  | свободно: <b>{ram_free if ram_free is not None else 'н/д'} MB</b>\n"
        f"💿 Disk: <b>{disk}</b>\n"
        f"⚙️ Активных тяжёлых задач: <b>{busy}</b>\n"
        f"🔄 Незавершённых workflow: <b>{workflows}</b>\n"
        f"🟢 Maintenance: <b>{'ON' if ADMIN_MAINTENANCE else 'OFF'}</b>"
    )

def _admin_stats_report() -> str:
    s = operation_stats if "operation_stats" in globals() else {}
    return (
        "📊 <b>ADMIN — статистика генераций</b>\n\n"
        f"Запросов: <b>{s.get('requests', 0)}</b>\n"
        f"Успешно: <b>{s.get('success', 0)}</b>\n"
        f"Ошибок: <b>{s.get('errors', 0)}</b>\n"
        f"📄 Word: <b>{s.get('word', 0)}</b>\n"
        f"📊 Excel: <b>{s.get('excel', 0)}</b>\n"
        f"📈 Презентаций: <b>{s.get('presentations', 0)}</b>\n"
        f"📚 Документов: <b>{s.get('documents', 0)}</b>\n"
        f"🎙 Voice: <b>{s.get('voice', 0)}</b>\n"
        f"🖼 Vision: <b>{s.get('vision', 0)}</b>"
    )

def _admin_user_report() -> str:
    try:
        total_users = len(users)
    except Exception:
        total_users = 0
    try:
        favorites_count = sum(len(v) for v in favorites.values())
    except Exception:
        favorites_count = 0
    try:
        ...
        artifacts_count = 0
    return (
        "👥 <b>ADMIN — пользователи и данные</b>\n\n"
        f"Пользователей: <b>{total_users}</b>\n"
        f"Избранных записей: <b>{favorites_count}</b>\n"
        f"Артефактов: <b>{artifacts_count}</b>\n"
        f"Audit events: <b>{sum(len(v) for v in ADMIN_AUDIT_LOG.values())}</b>"
    )
...
def _admin_help() -> str:
    return (
        ...
        "/health — состояние RAM/Disk/очередей\n"
        "/admin_stats — статистика генераций\n"
        "/admin_users — пользователи и артефакты\n"
        "/maintenance — включить/выключить техработы\n"
        "/admin_audit — последние действия администратора\n\n"
        "Опасные операции вроде массового удаления/рассылки "
...
async def _admin_panel_message(message: Message):
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "open_admin")
    await message.answer(_admin_runtime_report(), parse_mode="HTML",
...
async def admin_health(message: Message):
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "health")
    await message.answer(_admin_runtime_report(), parse_mode="HTML")
...
async def admin_stats(message: Message):
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "stats")
    await message.answer(_admin_stats_report(), parse_mode="HTML")
...
async def admin_users(message: Message):
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "users")
    await message.answer(_admin_user_report(), parse_mode="HTML")

@dp.message(Command("admin_audit"))
async def admin_audit(message: Message):
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "audit")
    rows = []
    for uid, events in list(ADMIN_AUDIT_LOG.items())[-20:]:
        for e in events[-5:]:
            rows.append(f"{uid}: {e['ts']} — {e['action']}")
    await message.answer(
        "🧾 <b>Последние admin-события</b>\n\n" +
        ("\n".join(rows[-50:]) if rows else "Пока нет событий."),
        parse_mode="HTML"
    )

@dp.message(Command("maintenance"))
async def admin_maintenance(message: Message):
    global ADMIN_MAINTENANCE
    if not _admin_is_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    ADMIN_MAINTENANCE = not ADMIN_MAINTENANCE
    _admin_log(message.from_user.id, "maintenance", str(ADMIN_MAINTENANCE))
    await message.answer(
        f"🛠 Maintenance: <b>{'ON' if ADMIN_MAINTENANCE else 'OFF'}</b>",
        parse_mode="HTML"
    )
...
async def admin_callback(call: CallbackQuery):
    ...
    _admin_log(call.from_user.id, f"callback:{action}")
    await call.answer()
    if action == "health":
        await call.message.edit_text(_admin_runtime_report(), parse_mode="HTML",
                                     reply_markup=_admin_keyboard())
    elif action == "stats":
        await call.message.edit_text(_admin_stats_report(), parse_mode="HTML",
                                     reply_markup=_admin_keyboard())
    elif action == "users":
        await call.message.edit_text(_admin_user_report(), parse_mode="HTML",
                                     reply_markup=_admin_keyboard())
    elif action == "audit":
        rows = []
        ...
        await call.message.edit_text(_admin_help(), parse_mode="HTML",
                                     reply_markup=_admin_keyboard())
# ======================= END ADMIN MAX CONTROL CENTER =======================




def _admin_user_blocked(user_id: int) -> bool:
    """True if a non-admin user is banned or muted."""
    if _admin_user_allowed(user_id):
        return False
    return user_id in ADMIN_BANNED_USERS
...
async def admin_user_profile_callback(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⬅️ Control", callback_data="sadm:dashboard")]
        ])
    )
...
async def admin_panel_button(message: Message):
    if not _admin_user_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    await message.answer(_admin_dashboard_text(), parse_mode="HTML", reply_markup=_serious_admin_keyboard())
...
async def admin_users_button(message: Message):
    ...
    await message.answer(
        "👥 <b>Пользователи</b>\n\n" + _admin_user_report() +
        "\n\nДля просмотра конкретного пользователя: <code>/user ID</code>\nИстория: <code>/history ID</code>",
        parse_mode="HTML"
    )

@dp.message(F.text == "📜 История")
async def admin_history_button(message: Message):
    if not _admin_user_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    await message.answer("📜 Укажи Telegram ID: <code>/history 123456789</code>", parse_mode="HTML")

# ===================== SERIOUS ADMIN CONTROL CENTER =====================
SERIOUS_ADMIN_CONTROL_CENTER = True
ADMIN_USER_STATE = {}
ADMIN_BANNED_USERS = set()
ADMIN_MUTED_USERS = set()
ADMIN_CUSTOM_LIMITS = {}
ADMIN_JOB_QUEUE = []
ADMIN_JOB_HISTORY = []
ADMIN_ALERTS = []
...
def _admin_user_allowed(uid: int) -> bool:
    try:
        return uid in ADMIN_IDS
    except Exception:
        return False

def _admin_find_user(uid_text: str):
    try:
        return int(uid_text.strip())
    except Exception:
        return None

def _admin_user_card(uid: int) -> str:
    try:
        u = users.get(uid, {})
    except Exception:
        u = {}
    return (
        ...
        f"Статус: <b>{'🚫 BAN' if uid in ADMIN_BANNED_USERS else '🔇 MUTE' if uid in ADMIN_MUTED_USERS else '🟢 ACTIVE'}</b>\n"
        f"Лимит: <b>{ADMIN_CUSTOM_LIMITS.get(uid, 'default')}</b>\n"
        f"Заметка: <b>{html.escape(ADMIN_NOTES.get(uid, '—'))}</b>\n"
        f"Артефактов: <b>{len(artifacts.get(uid, [])) if isinstance(artifacts, dict) else 0}</b>\n"
        f"Избранного: <b>{len(favorites.get(uid, [])) if isinstance(favorites, dict) else 0}</b>"
    )

def _admin_dashboard_text() -> str:
    health = get_runtime_health() if "get_runtime_health" in globals() else {}
    return (
        "🛡 <b>MecauAI ADMIN CONTROL CENTER</b>\n\n"
        f"🟢 Состояние: <b>{'MAINTENANCE' if globals().get('ADMIN_MAINTENANCE', False) else 'ONLINE'}</b>\n"
        f"💾 RAM: <b>{health.get('ram_percent', 'н/д')}%</b>\n"
        f"⚙️ Active jobs: <b>{health.get('busy', len(busy_users) if 'busy_users' in globals() else 0)}</b>\n"
        f"🔄 Workflows: <b>{health.get('workflows', 0)}</b>\n"
        f"👥 Users: <b>{len(users) if 'users' in globals() else 0}</b>\n"
        f"🛡 Admins: <b>{len(ADMIN_IDS)}</b>\n"
        f"🚫 Banned: <b>{len(ADMIN_BANNED_USERS)}</b>\n"
        f"🔇 Muted: <b>{len(ADMIN_MUTED_USERS)}</b>\n"
        f"📋 Queue: <b>{len(ADMIN_JOB_QUEUE)}</b>\n"
        f"⚠️ Alerts: <b>{len(ADMIN_ALERTS)}</b>"
    )

def _serious_admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Dashboard", callback_data="sadm:dashboard"),
         InlineKeyboardButton(text="🖥 Health", callback_data="sadm:health")],
        [InlineKeyboardButton(text="👥 Users", callback_data="sadm:users"),
         InlineKeyboardButton(text="🚫 Bans", callback_data="sadm:bans")],
        [InlineKeyboardButton(text="📋 Jobs", callback_data="sadm:jobs"),
         InlineKeyboardButton(text="⚠️ Alerts", callback_data="sadm:alerts")],
        [InlineKeyboardButton(text="📈 Metrics", callback_data="sadm:metrics"),
         InlineKeyboardButton(text="🛠 Maintenance", callback_data="sadm:maintenance")],
    ])

@dp.message(Command("control"))
async def serious_admin_control(message: Message):
    if not _admin_user_allowed(message.from_user.id):
        await message.answer("⛔ Доступ запрещён.")
        return
    _admin_log(message.from_user.id, "open_serious_control")
    await message.answer(_admin_dashboard_text(), parse_mode="HTML",
                         reply_markup=_serious_admin_keyboard())
...
async def admin_user_lookup(message: Message):
    ...
    if uid is None:
        await message.answer("⚠️ Некорректный ID пользователя.")
        return
    ADMIN_USER_STATE[message.from_user.id] = {"target": uid}
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🚫 Ban", callback_data=f"sadm:ban:{uid}"),
         InlineKeyboardButton(text="🟢 Unban", callback_data=f"sadm:unban:{uid}")],
        [InlineKeyboardButton(text="🔇 Mute", callback_data=f"sadm:mute:{uid}"),
         InlineKeyboardButton(text="🔊 Unmute", callback_data=f"sadm:unmute:{uid}")],
        [InlineKeyboardButton(text="🧹 Clear workflow", callback_data=f"sadm:clear:{uid}")],
        [InlineKeyboardButton(text="⬅️ Control", callback_data="sadm:dashboard")],
    ])
    await message.answer(_admin_user_card(uid), parse_mode="HTML", reply_markup=kb)
...
async def admin_unban(message: Message):
    ...
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) != 2:
        await message.answer("Использование: <code>/unban 123456789</code>", parse_mode="HTML")
        return
    uid = _admin_find_user(parts[1])
    if uid is None:
        await message.answer("⚠️ Некорректный ID.")
        return
    ADMIN_BANNED_USERS.discard(uid)
    _admin_log(message.from_user.id, "unban", str(uid))
...
async def admin_ban(message: Message):
    ...
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) != 2:
        await message.answer("Использование: <code>/ban 123456789</code>", parse_mode="HTML")
        return
    uid = _admin_find_user(parts[1])
    if uid is None or uid in ADMIN_IDS:
        await message.answer("⚠️ Нельзя заблокировать этого пользователя.")
        return
    ADMIN_BANNED_USERS.add(uid)
    _admin_log(message.from_user.id, "ban", str(uid))
...
async def admin_mute(message: Message):
    ...
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) != 2:
        await message.answer("Использование: <code>/mute 123456789</code>", parse_mode="HTML")
        return
    uid = _admin_find_user(parts[1])
    if uid is None or uid in ADMIN_IDS:
        await message.answer("⚠️ Нельзя заглушить этого пользователя.")
        return
    ADMIN_MUTED_USERS.add(uid)
    _admin_log(message.from_user.id, "mute", str(uid))
    await message.answer(f"🔇 Пользователь <code>{uid}</code> заглушен.", parse_mode="HTML")
...
async def admin_unmute(message: Message):
    ...
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) != 2:
        await message.answer("Использование: <code>/unmute 123456789</code>", parse_mode="HTML")
        return
    uid = _admin_find_user(parts[1])
    if uid is None:
        await message.answer("⚠️ Некорректный ID.")
        return
    ADMIN_MUTED_USERS.discard(uid)
    _admin_log(message.from_user.id, "unmute", str(uid))
    await message.answer(f"🔊 Пользователь <code>{uid}</code> снова активен.", parse_mode="HTML")

@dp.callback_query(F.data.startswith("sadm:"))
async def serious_admin_callback(call: CallbackQuery):
    if not _admin_user_allowed(call.from_user.id):
        await call.answer("Доступ запрещён", show_alert=True)
        return
    parts = call.data.split(":")
    action = parts[1]
    uid = _admin_find_user(parts[2]) if len(parts) > 2 else None
    await call.answer()
    if action == "dashboard":
        await call.message.edit_text(_admin_dashboard_text(), parse_mode="HTML",
                                     reply_markup=_serious_admin_keyboard())
    elif action == "health":
        await call.message.edit_text(_admin_runtime_report(), parse_mode="HTML",
                                     reply_markup=_serious_admin_keyboard())
        ...
        banned = "\n".join(f"• <code>{x}</code>" for x in sorted(ADMIN_BANNED_USERS)) or "Нет заблокированных."
        await call.message.edit_text("🚫 <b>BAN LIST</b>\n\n" + banned,
                                     parse_mode="HTML", reply_markup=_serious_admin_keyboard())
    elif action == "jobs":
        rows = []
        for job in ADMIN_JOB_HISTORY[-20:]:
            rows.append(f"• {html.escape(str(job))}")
        await call.message.edit_text("📋 <b>JOB HISTORY</b>\n\n" + ("\n".join(rows) or "Пусто."),
                                     parse_mode="HTML", reply_markup=_serious_admin_keyboard())
    elif action == "alerts":
        alerts = "\n".join(f"• {html.escape(str(x))}" for x in ADMIN_ALERTS[-20:]) or "Нет активных предупреждений."
        await call.message.edit_text("⚠️ <b>ALERTS</b>\n\n" + alerts,
                                     parse_mode="HTML", reply_markup=_serious_admin_keyboard())
    elif action == "metrics":
        await call.message.edit_text(_admin_stats_report(), parse_mode="HTML",
                                     reply_markup=_serious_admin_keyboard())
        ...
        await call.message.edit_text(_admin_dashboard_text(), parse_mode="HTML",
                                     reply_markup=_serious_admin_keyboard())
    elif action == "ban" and uid is not None:
        if uid not in ADMIN_IDS:
            ADMIN_BANNED_USERS.add(uid)
            _admin_log(call.from_user.id, "ban", str(uid))
        await call.message.edit_text(_admin_user_card(uid), parse_mode="HTML")
    elif action == "unban" and uid is not None:
        ADMIN_BANNED_USERS.discard(uid)
        _admin_log(call.from_user.id, "unban", str(uid))
        await call.message.edit_text(_admin_user_card(uid), parse_mode="HTML")
    elif action == "mute" and uid is not None:
        if uid not in ADMIN_IDS:
            ADMIN_MUTED_USERS.add(uid)
            _admin_log(call.from_user.id, "mute", str(uid))
        await call.message.edit_text(_admin_user_card(uid), parse_mode="HTML")
    elif action == "unmute" and uid is not None:
        ADMIN_MUTED_USERS.discard(uid)
        _admin_log(call.from_user.id, "unmute", str(uid))
        await call.message.edit_text(_admin_user_card(uid), parse_mode="HTML")
    elif action == "clear" and uid is not None:
        try:
            _reset_user_workflow(uid)
        except Exception:
            pass
        _admin_log(call.from_user.id, "clear_workflow", str(uid))
        await call.message.edit_text(_admin_user_card(uid), parse_mode="HTML")
