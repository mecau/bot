...
def get_documents_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📚 Анализ документа", callback_data="open_doc_analysis")],
        [InlineKeyboardButton(text="📄 Последний файл", callback_data="last_artifact_info")],
        [InlineKeyboardButton(text="🔄 Очистить контекст", callback_data="settings_clear")],
    ])
...
async def safe_answer(message: Message, text: str, parse_mode: str = None, reply_markup=None, disable_web_page_preview: bool = True):
    """Надёжная отправка: fallback при разметке + разбиение длинных ответов Telegram."""
    text = str(text or "")
    # Telegram ограничивает текст сообщения примерно 4096 символами.
    chunks=[]
    while len(text) > 3900:
        cut=text.rfind("\n",0,3900)
        if cut < 1200: cut=text.rfind(" ",0,3900)
        if cut < 1: cut=3900
    ...
    for i,chunk in enumerate(chunks):
        ...
        try:
            sent.append(await message.answer(chunk, parse_mode=parse_mode, reply_markup=markup, disable_web_page_preview=disable_web_page_preview))
        except Exception as e:
            logging.warning(f"Не удалось отправить сообщение с parse_mode={parse_mode}: {e}")
            plain_text = re.sub(r'<[^>]+>', '', chunk) if parse_mode == "HTML" else chunk
            try:
                sent.append(await message.answer(plain_text, reply_markup=markup, disable_web_page_preview=disable_web_page_preview))
...
async def cmd_documents_menu(message: Message):
    ...
    await message.answer(
        ...
        "Нажми «Анализ документа», затем отправь файл — после загрузки вопросы по нему можно задавать прямо в чате.",
        parse_mode="HTML",
...
async def handle_document(message: Message):
    ...
    if not allow_request(user_id, limit=5, window=60):
        ...
        return
    # Анализ документа теперь запускается автоматически: пользователь может
    # просто отправить PDF/DOCX/TXT, что соответствует обещанному UX «просто отправь файл».
...
@dp.callback_query(F.data == "open_doc_analysis")
async def cb_open_doc_analysis(callback: types.CallbackQuery):
    await callback.answer()
    await cmd_doc_analysis_prompt(callback.message)

@dp.callback_query(F.data == "last_artifact_info")
async def cb_last_artifact_info(callback: types.CallbackQuery):
    uid = callback.from_user.id
    art = last_artifact(uid)
    if not art:
        await callback.answer("Последних созданных файлов пока нет.", show_alert=True)
        return
    kind = {"pptx": "PowerPoint", "xlsx": "Excel", "docx": "Word"}.get(art.get("kind"), art.get("kind", "файл"))
    await callback.message.answer(f"📦 Последний файл: <b>{kind}</b>\n\nМожно написать: «измени последний слайд», «добавь диаграмму», «сделай подробнее» или другую конкретную правку.", parse_mode="HTML")
    await callback.answer()
    ...
    if user_id in ppt_states:
        ...
        if not topic:
            ...
            if user_id not in user_ppt_slide_count:
                ...
                return
            user_ppt_topic[user_id] = candidate[:2500]
            await message.answer(f"📌 Тема сохранена: «{candidate[:500]}»\n\nТеперь можешь отправить свои изображения или нажать «🚀 Начать генерацию».", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 Начать генерацию", callback_data="ppt_start")],[InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_flow")]]))
            return
        if normalized in {"готово", "готово!", "начать", "создавай", "генерируй"}:
            if user_id not in user_ppt_slide_count:
                await message.answer("📑 Сначала выбери количество слайдов через кнопки выше.")
                return
            await generate_presentation_file(message, topic, user_id)
            return
        await message.answer("📈 Презентация уже настроена. Отправь изображения, напиши «готово» или нажми «🚀 Начать генерацию».", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 Начать генерацию", callback_data="ppt_start")],[InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_flow")]]))
        return
...
def _edit_ppt_local(path,instruction):
    ...
    if re.search(r'последн(ий|ем)\s+слайд', low):
...
async def _edit_ppt_with_ai(path, instruction):
    prs = Presentation(path)
    if not prs.slides:
        raise ValueError("В презентации нет слайдов")
    slide = prs.slides[-1]
    current = "\n".join(sh.text for sh in slide.shapes if hasattr(sh, "text") and sh.text.strip())[:5000]
    response = await call_groq_with_retry(messages=[
        {"role": "system", "content": "Верни только JSON вида {title,points}. Перепиши последний слайд презентации по инструкции пользователя. title — короткий заголовок, points — 3-5 коротких тезисов. Не выдумывай факты, которых нет в исходном слайде или инструкции."},
        {"role": "user", "content": f"Исходный последний слайд:\n{current}\n\nИнструкция пользователя: {instruction[:2500]}"}
    ], temperature=0.25, max_retries=2, timeout=45)
    data = extract_json(clean_text_for_html(response.choices[0].message.content))
    item = _ppt_normalize_item({"title": data.get("title"), "points": data.get("points"), "layout": "bullets"}, len(prs.slides)-1)
    for shape in list(slide.shapes):
        _safe_remove_shape(shape)
    theme = PPT_THEMES["ppt_blue"]
    slide.background.fill.solid(); slide.background.fill.fore_color.rgb = theme["bg"]
    _ppt_add_header(slide, theme, len(prs.slides), item["title"])
    y = 1.55
    for i, pt in enumerate(item["points"][:5]):
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, PptxInches(.65), PptxInches(y), PptxInches(11.7), PptxInches(.82))
        card.fill.solid(); card.fill.fore_color.rgb = RGBColor(0xF6,0xF8,0xFA); card.line.color.rgb = RGBColor(0xE1,0xE6,0xEB)
        _ppt_add_text(slide, str(i+1), .88, y+.18, .35, .3, 9, True, theme["primary"], PP_ALIGN.CENTER)
        _ppt_add_text(slide, pt, 1.38, y+.12, 10.55, .58, 14 if len(pt)<130 else 12, False, theme["body_text"])
        y += .96
    _add_footer(slide, theme, len(prs.slides))
    _ppt_overflow_check(slide)
    bio=io.BytesIO(); prs.save(bio); _atomic_write_bytes(path,bio.getvalue())
...
async def handle_artifact_command(message, normalized):
    user_id=message.from_user.id
    # Не даём обходить общий access-control через команды редактирования файлов.
    if not await check_subscription(user_id):
        return False
    art=last_artifact(user_id)
    if not art: return False
    # Only intercept explicit edit/convert requests, not ordinary conversation.
    edit_words=('измени презентац','измени последний слайд','изменить последний слайд','измени файл','изменить файл','добавь слайд','убери слайд','удали слайд','убери график','удали график','добавь график','диаграмм','отсортируй','добавь столбец','добавь строк','расходы за','март','апрель','май','сделай подробнее','сделай более строг','официальн','добавь заголов')
    ...
    try:
        if art['kind']=='pptx':
            if re.search(r'последн(ий|ем)\s+слайд', normalized):
                await _edit_ppt_with_ai(art['path'], normalized)
            else:
                _edit_ppt_local(art['path'],normalized)
