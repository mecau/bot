...
def get_quick_actions_keyboard():
    """Основная компактная клавиатура пользователя."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="✨ Возможности"), KeyboardButton(text="🤖 Помощник")],
            [KeyboardButton(text="📂 Файлы"), KeyboardButton(text="📁 Документы")],
...
def keyboard_for(user_id: int):
    """Совместимый алиас основной клавиатуры.

    Старые обработчики используют keyboard_for(), поэтому оставляем единый
    источник клавиатуры через get_quick_actions_keyboard().
    """
...
async def check_subscription_details(user_id: int):
    """Свежая проверка членства во всех обязательных каналах. Никакого кэша."""
...
async def check_subscription(user_id: int, force: bool = False) -> bool:
    ok, _, _ = await check_subscription_details(user_id)
    return ok

async def require_subscription_callback(callback: types.CallbackQuery) -> bool:
    ok, failed, reason = await check_subscription_details(callback.from_user.id)
    if ok:
        return True
...
async def cb_check_sub(callback: types.CallbackQuery):
    ...
    ok, failed, reason = await check_subscription_details(user_id)
    if ok:
        save_user_id(user_id)
