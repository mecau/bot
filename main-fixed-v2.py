...
def _normalize_channel_ref(value):
    """Нормализует username/URL/ID канала для Telegram Bot API."""
    if value is None:
        return None
    raw = str(value).strip()
    if not raw:
    ...
    for prefix in ("https://t.me/", "http://t.me/", "https://telegram.me/", "http://telegram.me/"):
        if raw.startswith(prefix):
            raw = raw[len(prefix):].split("?", 1)[0].split("/", 1)[0]
            break
...
async def _resolve_channel(value):
    """Возвращает реальный chat_id канала и его данные."""
    ref = _normalize_channel_ref(value)
    if ref is None:
        raise ValueError("Пустой идентификатор канала")

    # Для username сначала получаем объект чата, затем используем его ID.
    # Это надёжнее, чем вручную собирать '@' + username.
    chat = await bot.get_chat(chat_id=ref)
    return chat


async def check_subscription(user_id: int, force: bool = False) -> bool:
    """Всегда делает свежую проверку membership во всех обязательных каналах.
    Кэш доступа отключён, чтобы подписка/отписка определялась без минутной задержки.
    При недавнем вступлении делаем до трёх быстрых попыток, т.к. Telegram может
    обновить membership с небольшой задержкой.
    """
    if user_id == MY_ADMIN_ID:
        return True

    channels = [("CHANNEL_1", CHANNEL_1_USERNAME), ("CHANNEL_2", CHANNEL_2_USERNAME)]
    for label, configured_ref in channels:
        subscribed = False
        for attempt in range(3):
            try:
                ...
                logging.info(
                    "SUB_CHECK user=%s channel=%s configured=%r chat_id=%s username=%r "
                    "status=%s is_member=%r result=%s attempt=%s",
                    user_id, label, configured_ref, chat.id,
                    getattr(chat, "username", None), status,
                    getattr(member, "is_member", None), subscribed, attempt + 1
                )
                if subscribed:
                    break
                if attempt < 2:
                ...
                logging.exception(
                    "SUB_CHECK_ERROR user=%s channel=%s configured=%r attempt=%s error=%s",
                    user_id, label, configured_ref, attempt + 1, e
                )
                if attempt < 2:
                    await asyncio.sleep(0.5)
        if not subscribed:
...
async def require_subscription_callback(callback: types.CallbackQuery) -> bool:
    """Единый guard для inline-кнопок, выполняющий свежую проверку доступа."""
    if await check_subscription(callback.from_user.id):
        return True
    try:
        await callback.answer("❌ Сначала подпишись на оба канала.", show_alert=True)
    except Exception:
        pass
    try:
...
async def cb_check_sub(callback: types.CallbackQuery):
    ...
    # ВАЖНО: после нажатия кнопки всегда делаем свежую проверку Telegram.
    if await check_subscription(user_id, force=True):
        save_user_id(user_id)
        try:
...
async def handle_text(message: Message):
    ...
    # Любой свободный текст, включая редактирование уже созданного файла,
    # проходит проверку подписки до выполнения действия.
    if not await check_subscription(user_id):
        await message.answer(
            f"🔒 Сначала подпишись на каналы:\n1️⃣ {CHANNEL_1_URL}\n2️⃣ {CHANNEL_2_URL}",
            reply_markup=get_sub_keyboard()
