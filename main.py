...
ADMIN_IDS_FILE = os.path.join(os.path.abspath(os.path.dirname(__file__) or "."), "admin_ids.json")
try:
...
async def handle_text(message: Message):
    ...
    # Выдача/отзыв админского доступа по Telegram ID.
    if user_id in admin_grant_states or -user_id in admin_grant_states:
        is_revoke = -user_id in admin_grant_states
        admin_grant_states.discard(user_id); admin_grant_states.discard(-user_id)
        raw = (message.text or "").strip()
        ...
        if target == MY_ADMIN_ID:
            await message.answer("ℹ️ Главного владельца изменить нельзя.")
            return
        if is_revoke:
            ADMIN_IDS.discard(target); _save_admin_ids(); _admin_log(user_id, "revoke_admin", str(target))
            await message.answer(f"✅ Доступ администратора забран у <code>{target}</code>.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")]]))
        else:
            ADMIN_IDS.add(target); _save_admin_ids(); _admin_log(user_id, "grant_admin", str(target))
            await message.answer(f"✅ Пользователь <code>{target}</code> теперь администратор.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")]]))
        return
    ...
    if user_id in admin_broadcast_states:
        ...
        for uid in load_user_ids():
            try:
                ...
                try:
                    await bot.send_message(uid, message.text, disable_web_page_preview=True); success += 1
                except Exception: failed += 1
            except Exception: failed += 1
            await asyncio.sleep(0.05)
        await status_msg.edit_text(f"✅ Рассылка завершена!\n\n👥 Доставлено: {success}\n❌ Ошибок: {failed}", reply_markup=_admin_back_keyboard())
        return
...
async def _admin_panel_message(message: Message):
    admin_grant_states.discard(message.from_user.id)
    admin_grant_states.discard(-message.from_user.id)
    admin_broadcast_states.discard(message.from_user.id)
