...
# ===== Persistent storage =====
# User data MUST NOT live beside main.py: the deployment rebuild can replace /app.
# Preferred location is /data. MECAUAI_DATA_DIR can explicitly point to another
...
def _choose_data_dir():
    ...
    # Bothost persistent volume is expected to be mounted here.
    # Prefer /data even when it has not been created yet; os.makedirs() below
    # will create it. If the platform does not mount it persistently, the
    # startup warning below makes that visible instead of silently pretending
    # that /app is safe.
    return "/data"
...
LEGAL_FILES = {
    "agreement": os.path.join(LEGAL_DIR, "agreement.pdf"),
    "privacy": os.path.join(LEGAL_DIR, "privacy.pdf"),
}

def get_legal_file(kind: str):
    path = LEGAL_FILES.get(kind)
    return path if path and os.path.isfile(path) else None

def check_legal_files():
    missing = [name for name, path in LEGAL_FILES.items() if not os.path.isfile(path)]
    if missing:
        logging.warning("Legal PDF files missing: %s (expected in %s)", ", ".join(missing), LEGAL_DIR)
    else:
        logging.info("Legal PDF files: OK (project root: %s)", LEGAL_DIR)

_APP_DIR = os.path.abspath(os.path.dirname(__file__) or ".")
_PERSISTENT_DATA_WARNING = (
    DATA_DIR == _APP_DIR
    or DATA_DIR.startswith(_APP_DIR + os.sep)
)

if _PERSISTENT_DATA_WARNING:
    logging.warning(
        "DATA_DIR=%s находится внутри директории приложения. "
        "При rebuild данные могут быть удалены. "
        "Для Bothost укажите MECAUAI_DATA_DIR=/data и подключите persistent volume.",
        DATA_DIR,
    )
else:
    logging.info("Persistent data directory: %s", DATA_DIR)
...
ACTIVITY_FILE = _data_file("user_activity.json")
ACTIVITY_MAX_PER_USER = int(os.getenv("MECAUAI_ACTIVITY_MAX_PER_USER", "300"))
activity_db = {}
activity_db_dirty = False
...
for _name in ("users.json", "favorites.json", "user_modes.json", "users_data.json", "user_stats.json", "user_activity.json", "artifacts.json", "admin_ids.json"):
    _migrate_legacy_file(_name)

# Migrate the old generated_files directory only for files that do not already
# exist in persistent storage. This is deliberately non-destructive.
_LEGACY_ARTIFACT_DIR = os.path.join(_APP_DIR, "generated_files")
_PERSISTENT_ARTIFACT_DIR = os.path.join(DATA_DIR, "generated_files")
if DATA_DIR != _APP_DIR and os.path.isdir(_LEGACY_ARTIFACT_DIR):
    try:
        import shutil as _shutil
        for _root, _dirs, _files in os.walk(_LEGACY_ARTIFACT_DIR):
            _rel = os.path.relpath(_root, _LEGACY_ARTIFACT_DIR)
            _dst_root = _PERSISTENT_ARTIFACT_DIR if _rel == "." else os.path.join(_PERSISTENT_ARTIFACT_DIR, _rel)
            os.makedirs(_dst_root, exist_ok=True)
            for _fn in _files:
                _src = os.path.join(_root, _fn)
                _dst = os.path.join(_dst_root, _fn)
                if not os.path.exists(_dst):
                    _shutil.copy2(_src, _dst)
        logging.info("Legacy generated_files migrated to %s", _PERSISTENT_ARTIFACT_DIR)
    except Exception as _exc:
        logging.warning("Не удалось перенести generated_files: %s", _exc)
...
def save_user_id(user_id: int):
    ...
    # Не делаем fsync на каждый входящий апдейт: это дорогая дисковая операция.
...
try:
    activity_db = {int(k): v for k, v in _load_json_dict(ACTIVITY_FILE).items()
                   if str(k).lstrip("-").isdigit() and isinstance(v, list)}
except Exception:
    activity_db = {}
...
ARTIFACT_DIR = os.path.abspath(
    os.getenv("MECAUAI_ARTIFACT_DIR", os.path.join(DATA_DIR, "generated_files"))
)
ARTIFACTS_FILE = os.path.abspath(
    os.getenv("MECAUAI_ARTIFACTS_FILE", os.path.join(DATA_DIR, "artifacts.json"))
)
...
def save_activity_db(immediate: bool = False):
    global activity_db_dirty
    if not immediate:
        activity_db_dirty = True
        return
    try:
        _atomic_json_write(ACTIVITY_FILE, {str(k): v[-ACTIVITY_MAX_PER_USER:] for k, v in activity_db.items()})
        activity_db_dirty = False
    except Exception as exc:
...
def flush_persistent_state(force: bool = False):
    global users_db_dirty, stats_db_dirty, favorites_dirty, users_ids_dirty, modes_db_dirty, artifacts_dirty, activity_db_dirty, last_persist_flush_at
    now = time.monotonic()
    ...
    if activity_db_dirty:
        save_activity_db(immediate=True)
    if favorites_dirty:
...
async def cb_help_and_settings(callback: types.CallbackQuery):
    ...
    if callback.data == "help_voice":
        ...
        await callback.message.edit_text("⚙️ <b>Настройки ИИ закрыты.</b>", parse_mode="HTML", reply_markup=keyboard_for(user_id))
...
@dp.message(F.text.in_({"✨ Возможности", "📁 Документы"}))
async def legacy_about_buttons(message: Message):
...
async def cmd_about(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📄 Документы", callback_data="about_documents")],
        [InlineKeyboardButton(text="❌ Закрыть", callback_data="about_close")],
    ])
    await message.answer(
        "ℹ️ <b>О MecauAI</b>\n\n"
        "Здесь собрана информация о сервисе, его возможностях и документах.",
        parse_mode="HTML",
        reply_markup=kb
    )


USER_AGREEMENT_TEXT = 'Пользовательское соглашение MecauAI\n\nРедакция от 14 августа 2026 года\n\nПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ\n\nсервиса MecauAI\n\nРедакция от 14 августа 2026 года\n\n1. ОБЩИЕ ПОЛОЖЕНИЯ\n\n1.1. Настоящее Пользовательское соглашение устанавливает правила использования сервиса MecauAI — Telegram-бота, предоставляющего доступ к функциям искусственного интеллекта и связанным инструментам.\n\n1.2. Используя MecauAI, пользователь подтверждает, что ознакомился с настоящим Соглашением и принимает его условия. Если пользователь не согласен с ними, он должен прекратить использование сервиса.\n\n2. СТАТУС И СТОИМОСТЬ\n\n2.1. На дату публикации настоящей редакции MecauAI предоставляется бесплатно.\n\n2.2. За доступный функционал плата с пользователя не взимается, если иное прямо не будет объявлено в будущем.\n\n2.3. Пользователь самостоятельно оплачивает интернет, мобильную связь и иные расходы, необходимые для доступа к Telegram и сервису.\n\n2.4. Бесплатность сервиса не означает гарантии постоянной доступности или сохранения любого конкретного функционала.\n\n3. ВОЗМОЖНОСТИ СЕРВИСА\n\n3.1. В зависимости от текущей версии MecauAI может предоставлять ответы ИИ, обработку текста и изображений, анализ документов, создание и редактирование файлов и иные функции.\n\n3.2. Состав функций может изменяться по мере развития проекта.\n\n4. ИСКУССТВЕННЫЙ ИНТЕЛЛЕКТ\n\n4.1. Результаты MecauAI формируются автоматически и могут содержать ошибки.\n\n4.2. Ошибки могут касаться фактов, дат, цифр, расчётов, формул, источников, выводов, кода, документов, презентаций, распознанного текста и результатов анализа изображений.\n\n4.3. MecauAI не гарантирует абсолютную точность, полноту или актуальность результата.\n\n4.4. Пользователь самостоятельно проверяет результат перед использованием.\n\n4.5. Ответы сервиса не являются медицинской, юридической, финансовой или иной профессиональной консультацией.\n\n5. МАТЕРИАЛЫ ПОЛЬЗОВАТЕЛЯ\n\n5.1. Пользователь самостоятельно решает, какие материалы передавать сервису.\n\n5.2. Пользователь отвечает за наличие законных оснований и необходимых прав на передаваемые материалы.\n\n5.3. Пользователь не должен передавать материалы, если их обработка нарушает законодательство РФ или права третьих лиц.\n\n6. СОЗДАННЫЕ ФАЙЛЫ\n\n6.1. Презентации, документы, таблицы и другие файлы создаются автоматически.\n\n6.2. Перед использованием пользователь самостоятельно проверяет текст, цифры, формулы, графики, изображения, ссылки, источники, выводы и оформление.\n\n7. ПРАВИЛА ИСПОЛЬЗОВАНИЯ\n\n7.1. MecauAI должен использоваться законным способом.\n\n7.2. Запрещается использовать сервис для противоправной деятельности, мошенничества, нарушения прав третьих лиц, распространения вредоносного ПО, незаконного получения или распространения персональных данных, вмешательства в работу сервиса, обхода технических ограничений и создания чрезмерной нагрузки.\n\n8. СТАБИЛЬНОСТЬ РАБОТЫ\n\n8.1. Работа MecauAI зависит от Telegram, серверной инфраструктуры, интернет-соединения и сторонних технологий.\n\n8.2. Возможны временная недоступность, задержки, ошибки обработки, технические работы и сбои сторонних сервисов.\n\n9. ОТВЕТСТВЕННОСТЬ\n\n9.1. Стороны несут ответственность в соответствии с законодательством РФ.\n\n9.2. Владелец не отвечает за решения пользователя, принятые на основании непроверенного результата ИИ, в той мере, в какой такое ограничение допускается законодательством.\n\n9.3. Настоящее Соглашение не исключает и не ограничивает ответственность, которая не может быть исключена или ограничена законом.\n\n10. ПЕРСОНАЛЬНЫЕ ДАННЫЕ\n\n10.1. Обработка персональных данных осуществляется в соответствии с отдельной Политикой в отношении обработки персональных данных MecauAI.\n\n10.2. Политика доступна через раздел «📄 Условия» и команду /documents.\n\n11. ИЗМЕНЕНИЕ СОГЛАШЕНИЯ\n\n11.1. Функции, интерфейс и технические процессы могут изменяться.\n\n11.2. Новая редакция Соглашения публикуется способом, доступным пользователям.\n\n12. КОНТАКТЫ\n\n12.1. Юсупов Вильдан Камилович.\n\n12.2. Telegram: @mecau.\n\n12.3. Email: denlox33@gmail.com.\n\n12.4. Telegram-бот: @mecau_ai_bot.\n\nДата редакции: 14 августа 2026 года.'
PRIVACY_POLICY_TEXT = 'Политика в отношении обработки персональных данных MecauAI\n\nРедакция от 14 августа 2026 года\n\nПОЛИТИКА В ОТНОШЕНИИ ОБРАБОТКИ ПЕРСОНАЛЬНЫХ ДАННЫХ\n\nсервиса MecauAI\n\nРедакция от 14 августа 2026 года\n\n1. ОБЩИЕ ПОЛОЖЕНИЯ\n\n1.1. Настоящая Политика определяет порядок обработки персональных данных при использовании Telegram-бота MecauAI.\n\n1.2. Политика разработана с учётом Федерального закона от 27.07.2006 № 152-ФЗ «О персональных данных».\n\n2. КАКИЕ ДАННЫЕ ОБРАБАТЫВАЮТСЯ\n\n2.1. В зависимости от использования сервиса могут обрабатываться:\n\nTelegram ID;\n\nимя и username, доступные Telegram;\n\nсодержание сообщений и запросов;\n\nистория текущего диалога и сохранённый контекст;\n\nдокументы, изображения и фотографии;\n\nголосовые сообщения и их расшифровка;\n\nнастройки пользователя;\n\nсведения о созданных файлах и технические идентификаторы;\n\nтехническая и статистическая информация.\n\n3. ЦЕЛИ ОБРАБОТКИ\n\n3.1. Данные обрабатываются для предоставления функций MecauAI, обработки запросов, анализа материалов, создания файлов, сохранения контекста, проверки обязательной подписки, предотвращения злоупотреблений, диагностики ошибок, технической статистики и исполнения обязанностей по закону.\n\n4. ПРАВОВЫЕ ОСНОВАНИЯ\n\n4.1. Обработка осуществляется на основаниях, предусмотренных законодательством РФ, включая случаи, когда она необходима для исполнения соглашения с пользователем, выполнения обязанностей по закону либо основана на согласии пользователя, когда такое согласие требуется.\n\n5. КАК ОБРАБАТЫВАЮТСЯ ДАННЫЕ\n\n5.1. В зависимости от задачи могут выполняться получение, запись, систематизация, хранение, использование, передача для выполнения конкретной функции, изменение, удаление и уничтожение данных.\n\n5.2. Часть данных хранится на серверной инфраструктуре сервиса.\n\n5.3. Отдельные данные могут временно передаваться внешним технологическим сервисам для выполнения конкретной функции.\n\n6. СТОРОННИЕ СЕРВИСЫ\n\n6.1. Для работы MecauAI используются Telegram и сторонние технологические сервисы.\n\n6.2. В текущей реализации для AI-обработки используются технологии Groq, а для отдельных функций генерации изображений может использоваться Pollinations AI.\n\n6.3. В зависимости от функции внешнему сервису может быть передан необходимый для её выполнения пользовательский контент.\n\n6.4. При использовании внешних сервисов возможна передача данных за пределы Российской Федерации.\n\n7. ХРАНЕНИЕ И УДАЛЕНИЕ\n\n7.1. Срок хранения зависит от цели и конкретной технической функции.\n\n7.2. Данные, необходимые для продолжения работы пользователя, могут храниться в хранилище сервиса.\n\n7.3. Временные файлы и промежуточные данные могут удаляться после завершения операции или технического обслуживания.\n\n7.4. После достижения целей обработки данные удаляются или уничтожаются, если отсутствует законное основание для дальнейшего хранения.\n\n8. ЗАЩИТА ДАННЫХ\n\n8.1. Оператор принимает разумные организационные и технические меры для защиты данных.\n\n8.2. Абсолютная безопасность информации, передаваемой через Интернет и сторонние инфраструктуры, не может быть гарантирована.\n\n9. ПРАВА ПОЛЬЗОВАТЕЛЯ\n\n9.1. Пользователь вправе обратиться к Оператору по вопросам обработки своих персональных данных.\n\n9.2. В предусмотренных законом случаях пользователь вправе требовать уточнения, блокирования или удаления данных и получать сведения об их обработке.\n\n10. ДАННЫЕ ТРЕТЬИХ ЛИЦ\n\n10.1. Пользователь не должен передавать сервису персональные данные других лиц без законного основания.\n\n11. ИЗМЕНЕНИЕ ПОЛИТИКИ\n\n11.1. Политика может обновляться при изменении законодательства, архитектуры сервиса или порядка обработки данных.\n\n11.2. Актуальная редакция доступна через раздел «📄 Условия» и команду /documents.\n\n12. КОНТАКТЫ\n\n12.1. Юсупов Вильдан Камилович.\n\n12.2. Telegram: @mecau.\n\n12.3. Email: denlox33@gmail.com.\n\n12.4. Telegram-бот: @mecau_ai_bot.\n\nДата редакции: 14 августа 2026 года.'


def _split_telegram_text(text: str, limit: int = 3800):
    text = text.strip()
    return [text[i:i + limit] for i in range(0, len(text), limit)] or [""]


def _about_documents_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📄 Пользовательское соглашение", callback_data="legal_agreement")],
...
def _about_features_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="about_back")],
    ])


async def _show_about_features(message: Message):
    await message.answer(
        "✨ <b>Возможности MecauAI</b>\n\n"
        "• 🧠 ответы ИИ и работа с контекстом\n"
        "• 🎙 голосовые сообщения\n"
        "• 🖼 анализ изображений\n"
        "• 📚 анализ PDF, DOCX, TXT и материалов\n"
        "• 📈 создание презентаций\n"
        "• 📊 создание и обработка Excel\n"
        "• 📄 создание и редактирование Word\n"
        "• ⭐ избранное и дополнительные инструменты\n\n"
        "Просто отправь задачу — бот сам определяет подходящий способ обработки.",
        parse_mode="HTML",
        reply_markup=_about_features_keyboard()
    )


async def _show_about_documents(message: Message):
    await message.answer(
        "📄 <b>Документы MecauAI</b>\n\n"
        "Здесь доступны актуальные юридические документы сервиса.",
        parse_mode="HTML",
        reply_markup=_about_documents_keyboard()
    )


@dp.callback_query(F.data.in_({
    "about_features", "about_documents", "about_back",
    "about_close", "legal_agreement", "legal_privacy"
}))
async def cb_about_and_documents(callback: types.CallbackQuery):
    if not await require_subscription_callback(callback):
        return

    if callback.data == "about_features":
        await _show_about_features(callback.message)

    elif callback.data == "about_documents":
        await _show_about_documents(callback.message)

    elif callback.data == "about_back":
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✨ Возможности", callback_data="about_features")],
            [InlineKeyboardButton(text="📄 Документы", callback_data="about_documents")],
            [InlineKeyboardButton(text="❌ Закрыть", callback_data="about_close")],
    ...
    elif callback.data == "about_close":
        try:
            await callback.message.delete()
        except Exception:
        ...
        kind = "agreement" if callback.data == "legal_agreement" else "privacy"
        path = get_legal_file(kind)
        if not path:
            await callback.message.answer(
                "⚠️ Документ пока недоступен. Попробуйте позже."
            )
        else:
...
async def cmd_documents(message: Message):
    if not await check_subscription(message.from_user.id):
        ...
        return
    await _show_about_documents(message)
...
async def handle_document(message: Message):
    ...
    _track_user_event(user_id, message, kind="document", prompt=message.document.file_name if message.document else '')
...
MENU_BUTTONS = {
    "🛠 Техподдержка", "⭐ Избранное", "⚙️ Настройки ИИ",
...
async def handle_voice(message: Message):
    ...
    save_user_id(user_id)
    get_user_data(user_id)
    _track_user_event(user_id, message, kind="voice", prompt="Голосовой запрос")
...
async def handle_photo(message: Message):
    ...
    save_user_id(user_id)
    get_user_data(user_id)
    _track_user_event(user_id, message, kind="vision", prompt="")
...
async def handle_text(message: Message):
    ...
    save_user_id(user_id)
    get_user_data(user_id)
    _track_user_event(user_id, message, kind="text", prompt=message.text or "")
    ...
    if user_id in ppt_states:
        ...
        if not current_topic:
            ...
            await message.answer(
                ...
                "Когда закончишь, напиши <b>«готово»</b>. Если картинки не нужны — "
                "сразу напиши <b>«готово»</b>.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await message.answer(
            "🖼 Тема уже принята. Отправь свои изображения или напиши "
            "<b>«готово»</b>, чтобы начать создание презентации.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
def _admin_runtime_report() -> str:
    ...
    return (
        ...
        f"👥 Пользователей: <b>{len(load_user_ids())}</b>\n"
...
def _admin_server_report() -> str:
    ram = _memory_mb()
    disk = _disk_free_mb()
    return (
        "🖥 <b>СЕРВЕР</b>\n\n"
        f"💾 RAM процесса: <b>{ram:.0f} MB</b>\n"
        f"📦 Лимит RAM: <b>{round(_CONTAINER_RAM_MB) if _CONTAINER_RAM_MB else 'н/д'} MB</b>\n"
        f"💿 Свободно на диске: <b>{f'{disk:.0f} MB' if disk is not None else 'н/д'}</b>\n"
        f"⚙️ AI-параллельность: <b>{MAX_AI_CONCURRENCY}</b>\n"
        f"🧰 Тяжёлые задачи: <b>{MAX_HEAVY_CONCURRENCY}</b>\n"
        f"⏳ Очередь: <b>{HEAVY_WAITERS}/{MAX_HEAVY_QUEUE}</b>"
    )

def _admin_health_report() -> str:
    return (
        "🚨 <b>СОСТОЯНИЕ</b>\n\n"
        f"🟢 Бот: <b>{'ТЕХРАБОТЫ' if ADMIN_MAINTENANCE else 'ONLINE'}</b>\n"
        f"👥 Пользователей: <b>{len(load_user_ids())}</b>\n"
        f"⚙️ Активных задач: <b>{len(busy_users)}</b>\n"
        f"📚 История пользователей: <b>{'OK' if os.path.exists(ACTIVITY_FILE) or activity_db else 'ещё не создана'}</b>\n"
        f"📄 PDF документов: <b>{sum(1 for x in LEGAL_FILES.values() if os.path.isfile(x))}/2</b>\n"
        f"💾 Сохранение: <b>автоматически</b>"
    )
...
def _admin_back_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")]])
...
def _admin_admins_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")],
    ])
...
async def _show_admin_users(message: Message, page: int = 0):
    ids = sorted(load_user_ids(), reverse=True)
    per_page = 8
    ...
    for uid in chunk:
        ...
        name = " ".join(x for x in [p.get("first_name"), p.get("last_name")] if x) or "Без имени"
        username = f"@{p['username']}" if p.get("username") else "без username"
        last = USER_ACTIVITY_INDEX.get(uid, "—")
        rows.append([InlineKeyboardButton(
            text=f"👤 {name[:24]} · {username[:18]}",
            callback_data=f"up:{uid}"
        )])
    nav=[]
    if page > 0: nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"adm:users:{page-1}"))
    if page < pages-1: nav.append(InlineKeyboardButton(text="➡️", callback_data=f"adm:users:{page+1}"))
    if nav: rows.append(nav)
    rows.append([InlineKeyboardButton(text="📋 Последняя активность", callback_data="adm:audit:0")])
    rows.append([InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")])
    text = f"👥 <b>ПОЛЬЗОВАТЕЛИ</b>\nВсего: <b>{len(ids)}</b>\nСтраница: <b>{page+1}/{pages}</b>\n\n"
    text += "\n".join(f"• <code>{uid}</code> · {html.escape((USER_PROFILE_CACHE.get(uid, {}).get('username') and '@'+USER_PROFILE_CACHE.get(uid, {}).get('username')) or '—')} · {html.escape(USER_ACTIVITY_INDEX.get(uid,'—'))}" for uid in chunk) or "Пока нет пользователей."
    await message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))

async def _show_admin_activity(message: Message, page: int = 0):
    events=[]
    for uid, rows in activity_db.items():
        for e in rows[-20:]:
            events.append((e.get("ts",""), uid, e))
    ...
    lines=[]
    for ts,uid,e in chunk:
        p=e.get("prompt") or "—"
        if len(p)>140: p=p[:137]+"..."
        name=e.get("first_name") or (f"@{e['username']}" if e.get("username") else str(uid))
        lines.append(f"• <b>{html.escape(ts)}</b> · <code>{uid}</code> · {html.escape(str(name))}\n  {html.escape(str(e.get('kind','request')))}: {html.escape(p)}")
    rows=[]
    nav=[]
    if page>0: nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"adm:audit:{page-1}"))
    if page<pages-1: nav.append(InlineKeyboardButton(text="➡️", callback_data=f"adm:audit:{page+1}"))
    if nav: rows.append(nav)
    rows.append([InlineKeyboardButton(text="👥 Пользователи", callback_data="adm:users:0")])
    rows.append([InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm:dashboard")])
    await message.edit_text(
        f"📋 <b>ИСТОРИЯ ПОЛЬЗОВАТЕЛЕЙ</b>\nСтраница: <b>{page+1}/{pages}</b>\n\n" + ("\n\n".join(lines) or "Пока нет истории."),
...
async def admin_callback(call: CallbackQuery):
    ...
    _admin_log(call.from_user.id, "callback", call.data)
    if action == "dashboard":
        ...
        await call.message.edit_text(_admin_server_report(), parse_mode="HTML", reply_markup=_admin_back_keyboard())
    elif action == "users":
        await _show_admin_users(call.message, page)
    elif action == "audit":
        await _show_admin_activity(call.message, page)
    elif action == "admins":
        ...
        await call.message.edit_text(_admin_health_report(), parse_mode="HTML", reply_markup=_admin_back_keyboard())
...
def _track_user_event(user_id: int, message=None, kind="request", status="ok",
    ...
    """Persistent lightweight per-user history. Stores metadata and a short prompt."""
    global activity_db_dirty
    try:
        u = getattr(message, "from_user", None) if message is not None else None
        profile = USER_PROFILE_CACHE.setdefault(int(user_id), {})
        if u:
            profile.update({
                "id": int(user_id),
        ...
        rows = activity_db.setdefault(int(user_id), [])
        rows.append(event)
        if len(rows) > ACTIVITY_MAX_PER_USER:
            del rows[:-ACTIVITY_MAX_PER_USER]
        USER_REQUEST_HISTORY[int(user_id)] = rows
        USER_ACTIVITY_INDEX[int(user_id)] = event["ts"]
        activity_db_dirty = True
    except Exception as exc:
        logging.debug("activity tracking failed: %s", exc)

# Restore in-memory history from persistent storage after definitions.
USER_REQUEST_HISTORY.update(activity_db)
for _uid, _rows in activity_db.items():
    if _rows:
        USER_ACTIVITY_INDEX[_uid] = _rows[-1].get("ts", "—")
        if _rows[-1].get("username") or _rows[-1].get("first_name"):
            USER_PROFILE_CACHE[_uid] = {
                "id": _uid,
                "username": _rows[-1].get("username"),
                "first_name": _rows[-1].get("first_name"),
            }
...
async def admin_user_profile_callback(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⬅️ Control", callback_data="adm:dashboard")]
        ])
    )
...
async def main():
    ...
    cleanup_artifacts()
    logging.info("Persistent user history: %s (%d users)", ACTIVITY_FILE, len(activity_db))
    _cleanup_orphan_artifact_files()
    logging.info(
