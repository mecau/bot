...
logging.info("MecauAI persistent data directory: %s", DATA_DIR)
if DATA_DIR == os.path.abspath(os.path.dirname(__file__) or "."):
    logging.warning("DATA_DIR находится рядом с main.py. Для сохранения данных при пересборке задайте MECAUAI_DATA_DIR на persistent volume (например /data).")
...
USER_PROFILES_FILE = _data_file("user_profiles.json")
USER_ACTIVITY_FILE = _data_file("user_activity.json")
USER_ACTIVITY_MAX_PER_USER = int(os.getenv("MECAUAI_ACTIVITY_MAX_PER_USER", "300"))
...
for _name in ("users.json", "favorites.json", "user_modes.json", "users_data.json", "user_stats.json", "artifacts.json", "user_profiles.json", "user_activity.json"):
...
def save_user_id(user_id: int):
    ...
    # Пользовательский ID критичен: сохраняем сразу атомарно.
    # Это предотвращает потерю нового пользователя при внезапном рестарте/пересборке.
    try:
        _atomic_json_write(USERS_FILE, sorted(all_users_cache))
        users_ids_dirty = False
    except Exception as exc:
        users_ids_dirty = True
        logging.error("Не удалось немедленно сохранить пользователя %s: %s", user_id, exc)

all_users_cache = load_user_ids()

def _recover_user_ids_from_all_data():
    """Восстанавливает ID из всех доступных постоянных хранилищ.
    Это защищает счётчик пользователей при обновлении main.py.
    """
    recovered = set(all_users_cache)
    sources = [
        USER_DATA_FILE,
        STATS_FILE,
        MODES_FILE,
    ...
    for path in sources:
        data = _load_json_dict(path) if os.path.exists(path) else {}
        if isinstance(data, dict):
            for key in data.keys():
                try:
                    recovered.add(int(key))
                except (TypeError, ValueError):
                    continue
    return recovered
...
# Восстанавливаем пользователей после загрузки всех JSON.
try:
    all_users_cache = _recover_user_ids_from_all_data()
except Exception as exc:
    logging.warning("Не удалось полностью восстановить список пользователей: %s", exc)
...
def get_answer_inline_keyboard():
    """Компактные действия под ответом; второстепенные преобразования спрятаны в «Улучшить»."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="💡 Проще", callback_data="btn_simplify"),
            InlineKeyboardButton(text="✨ Улучшить", callback_data="btn_improve_menu"),
...
def get_answer_improve_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
        ...
        [InlineKeyboardButton(text="◀️ Назад", callback_data="btn_improve_back")],
    ])
...
def get_settings_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📊 Моя активность", callback_data="settings_mydata")],
...
async def cmd_start(message: Message):
    ...
    _track_user_event(user_id, message=message, kind="start", prompt="/start", status="started")
...
@dp.callback_query(F.data.in_({"help_voice", "help_file", "help_ppt", "help_excel", "help_mode",
                               "settings_mode", "settings_mydata", "settings_clear", "settings_close"}))
async def cb_help_and_settings(callback: types.CallbackQuery):
    ...
    elif callback.data == "settings_mydata":
        history = USER_REQUEST_HISTORY.get(user_id, [])
        by_kind = {}
        for row in history:
            k = row.get("kind", "request")
            by_kind[k] = by_kind.get(k, 0) + 1
        await callback.message.answer(
            "📊 <b>Моя активность</b>\n\n"
            f"Всего действий: <b>{len(history)}</b>\n"
            f"Последняя активность: <b>{html.escape(str(USER_ACTIVITY_INDEX.get(user_id, '—')))}</b>\n"
            f"🎙 Голос: <b>{by_kind.get('voice', 0)}</b>  "
            f"🖼 Фото: <b>{by_kind.get('vision', 0)}</b>\n"
            f"📚 Документы: <b>{by_kind.get('document', 0)}</b>  "
            f"💬 Запросы: <b>{by_kind.get('request', 0)}</b>\n\n"
            "История хранит только технические сведения о работе бота и не сохраняет содержимое загруженных файлов.",
            parse_mode="HTML"
        )
...
async def cmd_mydata(message: Message):
    ...
    await message.answer(
        "📊 <b>Моя активность</b>\n\n"
        f"Запросов за всё время в доступной истории: <b>{len(history)}</b>\n"
        f"Последняя активность: <b>{html.escape(str(USER_ACTIVITY_INDEX.get(uid, '—')))}</b>\n\n"
        "История хранится как техническая статистика работы с ботом; содержимое файлов не сохраняется в эту историю.",
        parse_mode="HTML",
        reply_markup=get_cancel_keyboard()
    )
...
async def handle_document(message: Message):
    _track_user_event(message.from_user.id, message=message, kind="document", prompt=(message.caption or "Загрузка документа")[:1500], status="started")
    user_id = message.from_user.id
    if not await check_subscription(user_id):
        await message.answer(f"🔒 Сначала подпишись на каналы:\n1️⃣ {CHANNEL_1_URL}\n2️⃣ {CHANNEL_2_URL}", reply_markup=get_sub_keyboard())
        return
...
@dp.callback_query(F.data == "btn_improve_menu")
async def cb_answer_improve_menu(callback: types.CallbackQuery):
    if not await require_subscription_callback(callback):
        return
    await callback.message.edit_reply_markup(reply_markup=get_answer_improve_keyboard())
    await callback.answer()

@dp.callback_query(F.data == "btn_improve_back")
async def cb_answer_improve_back(callback: types.CallbackQuery):
    if not await require_subscription_callback(callback):
        return
    await callback.message.edit_reply_markup(reply_markup=get_answer_inline_keyboard())
    await callback.answer()
...
async def handle_voice(message: Message):
    _track_user_event(message.from_user.id, message=message, kind="voice", prompt="Голосовое сообщение", status="started")
    """
...
async def handle_photo(message: Message):
    _track_user_event(message.from_user.id, message=message, kind="vision", prompt=(message.caption or "Изображение")[:1500], status="started")
    user_id = message.from_user.id
    if not await check_subscription(user_id):
        await message.answer(f"🔒 Сначала подпишись на каналы:\n1️⃣ {CHANNEL_1_URL}\n2️⃣ {CHANNEL_2_URL}", reply_markup=get_sub_keyboard())
        return
...
async def handle_text(message: Message):
    _track_user_event(message.from_user.id, message=message, kind="request", prompt=(message.text or "")[:1500], status="started")
    user_id = message.from_user.id
...
def _load_user_analytics():
    global USER_PROFILE_CACHE, USER_REQUEST_HISTORY, USER_ACTIVITY_INDEX
    profiles = _load_json_dict(USER_PROFILES_FILE)
    activity = _load_json_dict(USER_ACTIVITY_FILE)
    USER_PROFILE_CACHE = {}
    USER_REQUEST_HISTORY = {}
    USER_ACTIVITY_INDEX = {}
    ...
    for key, value in activity.items():
        ...
        if isinstance(value, list):
            rows = [x for x in value if isinstance(x, dict)][-USER_ACTIVITY_MAX_PER_USER:]
            USER_REQUEST_HISTORY[uid] = rows
            if rows:
                USER_ACTIVITY_INDEX[uid] = rows[-1].get("ts", "—")

def _save_user_analytics():
    try:
        _atomic_json_write(USER_PROFILES_FILE, {str(k): v for k, v in USER_PROFILE_CACHE.items()})
        _atomic_json_write(USER_ACTIVITY_FILE, {str(k): v[-USER_ACTIVITY_MAX_PER_USER:] for k, v in USER_REQUEST_HISTORY.items()})
    except Exception as exc:
        logging.error("Не удалось сохранить историю пользователей: %s", exc)

_load_user_analytics()
...
def _track_user_event(user_id: int, message=None, kind="request", status="ok",
    ...
    try:
        ...
        _save_user_analytics()
    except Exception as exc:
        logging.debug("Не удалось записать активность пользователя %s: %s", user_id, exc)
