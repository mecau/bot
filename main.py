...
async def _refresh_available_models():
    """Best-effort startup check of models visible to this Groq key/project."""
    try:
        result = await groq_client.models.list()
        ids = {getattr(m, "id", None) for m in getattr(result, "data", [])}
        ids = {x for x in ids if x}
        if ids:
            global TEXT_MODEL_FALLBACKS, VISION_MODEL_FALLBACKS
            text_available = [m for m in TEXT_MODEL_FALLBACKS if m in ids]
            vision_available = [m for m in VISION_MODEL_FALLBACKS if m in ids]
            if text_available:
                TEXT_MODEL_FALLBACKS = text_available
            if vision_available:
                VISION_MODEL_FALLBACKS = vision_available
            logging.info("Groq visible models: text=%s vision=%s", TEXT_MODEL_FALLBACKS, VISION_MODEL_FALLBACKS)
        else:
            logging.warning("Groq models.list() вернул пустой список; оставляю configured fallbacks")
    except Exception as exc:
        logging.warning("Не удалось получить список моделей Groq при старте: %s", exc)
...
async def cb_ppt_count(callback: types.CallbackQuery):
    ...
    if topic:
        await callback.message.answer(f"📌 <b>Шаг 3/3 — тема уже получена:</b> «{topic[:300]}»\n\nОтправь свои картинки или сразу нажми «🚀 Начать генерацию».",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 Начать генерацию",callback_data="ppt_start")],[InlineKeyboardButton(text="❌ Отмена",callback_data="cancel_flow")]]))
    else:
        await callback.message.answer("📌 <b>Шаг 3/3 — отправь тему презентации.</b>\n\nПосле темы можно добавить свои изображения или сразу нажать кнопку создания.",parse_mode="HTML")
    await callback.answer()
...
async def generate_presentation_file(message: Message, topic: str, user_id: int = None):
    ...
    theme = PPT_THEMES.get("ppt_blue", next(iter(PPT_THEMES.values())))
    try:
...
def _admin_users_text(page=0, per_page=8):
    ...
    if not chunk:
        ...
        for uid in chunk:
            ...
            name = " ".join(x for x in [first, last_name] if x).strip()
            username_raw = str(p.get("username") or "").strip()
...
def _admin_users_keyboard(page=0):
    ...
    for uid in chunk:
        ...
        if not isinstance(p, dict):
            p = {}
        label = " ".join(str(x).strip() for x in [p.get("first_name"), p.get("last_name")] if x).strip()
        if not label:
            label = f"ID {uid}"
...
async def admin_users_page_callback(call: CallbackQuery):
    ...
    try:
        ...
        keyboard = _admin_users_keyboard(page)
        await call.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    except Exception as exc:
        logging.exception("Ошибка открытия списка пользователей: %s", exc)
        try:
            await call.answer("⚠️ Не удалось открыть список пользователей", show_alert=True)
        except Exception:
            pass
...
async def admin_callback(call: CallbackQuery):
    ...
    if action in {"dashboard", "back"}:
        ...
        try:
            text = _admin_users_text(0)
            keyboard = _admin_users_keyboard(0)
            await call.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
        except Exception as exc:
            logging.exception("Ошибка открытия пользователей из adm: %s", exc)
            try:
                await call.answer("⚠️ Не удалось открыть список пользователей", show_alert=True)
            except Exception:
                pass
...
def _load_user_activity():
    ...
    for uid_s, item in data.items():
        ...
        USER_PROFILE_CACHE[uid] = profile if isinstance(profile, dict) else {}
...
def _all_known_user_ids() -> set:
    """Единый безопасный источник пользователей для админки."""
    ids = set()
    ...
    add_ids(all_users_cache)
    add_ids(users_db)
    add_ids(user_stats)
    add_ids(USER_PROFILE_CACHE)
    add_ids(USER_REQUEST_HISTORY)
    return ids
...
async def admin_user_history_callback(call: CallbackQuery):
    ...
    try:
        ...
        logging.exception("Ошибка открытия истории пользователя: %s", exc)
        try:
            await call.answer("⚠️ Не удалось открыть историю", show_alert=True)
        except Exception:
            pass
...
async def admin_user_profile_callback(call: CallbackQuery):
    ...
    try:
        ...
        logging.exception("Ошибка открытия профиля пользователя: %s", exc)
        try:
            await call.answer("⚠️ Не удалось открыть профиль", show_alert=True)
        except Exception:
            pass
...
async def main():
    ...
    logging.info("Groq models: text=%s fallbacks=%s vision=%s fallbacks=%s", TEXT_MODEL, TEXT_MODEL_FALLBACKS, VISION_MODEL, VISION_MODEL_FALLBACKS)
