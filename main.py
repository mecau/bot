...
async def check_subscription(user_id: int, force: bool = False) -> bool:
    ...
    force=True используется кнопкой «Проверить подписку» и полностью
    игнорирует кэш, чтобы подписка определялась сразу после вступления.
    """
    ...
    channels = [
        ("CHANNEL_1", CHANNEL_1_USERNAME),
        ("CHANNEL_2", CHANNEL_2_USERNAME),
    ]
    ...
    for label, configured_ref in channels:
        try:
            ...
            # member/administrator/creator — обычные подписчики и админы.
            # restricted может означать, что пользователь ограничен,
            # но всё ещё состоит в канале; проверяем is_member.
            is_subscribed = (
            ...
            logging.info(
                "SUB_CHECK user=%s channel=%s configured=%r chat_id=%s "
                "chat_username=%r status=%s is_member=%r result=%s",
                user_id,
                label,
                configured_ref,
                chat.id,
            ...
            if not is_subscribed:
                # Отрицательный результат НЕ кэшируем.
                subscription_cache.pop(user_id, None)
                return False

        except Exception as e:
            logging.exception(
                "SUB_CHECK_ERROR user=%s channel=%s configured=%r error=%s",
                user_id,
                label,
                configured_ref,
                e,
            ...
            # Не превращаем техническую ошибку Telegram в вечный False.
            # Если совсем недавно была подтверждённая подписка — сохраняем её
            # на короткое время, иначе запрещаем доступ до следующей проверки.
            if cached and cached[0] and now - cached[1] < 30:
                return True
