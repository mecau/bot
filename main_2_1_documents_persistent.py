...
async def cb_about_and_documents(callback: types.CallbackQuery):
    ...
    if callback.data == "about_features":
        ...
        text = USER_AGREEMENT_TEXT if callback.data == "legal_agreement" else PRIVACY_POLICY_TEXT
        title = "📄 Пользовательское соглашение" if callback.data == "legal_agreement" else "🔐 Политика конфиденциальности"
        for idx, chunk in enumerate(_split_telegram_text(text)):
            prefix = f"<b>{title}</b>\n\n" if idx == 0 else ""
            await callback.message.answer(prefix + html.escape(chunk), parse_mode="HTML")
