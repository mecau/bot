...
def _backup_persistent_user_data():
    """Snapshot user data before an update touches persistent state."""
    try:
        backup_dir = os.path.join(DATA_DIR, "backups")
        os.makedirs(backup_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        for name in ("users.json", "users_data.json", "user_stats.json", "user_profiles.json"):
            src_path = _data_file(name)
        ...
        logging.warning("User-data backup failed: %s", exc)

_backup_persistent_user_data()
...
for _name in ("users.json", "favorites.json", "user_modes.json", "users_data.json", "user_stats.json", "artifacts.json", "user_activity.json", "user_profiles.json"):
...
def save_user_id(user_id: int):
    ...
    try:
        ...
        logging.warning("Не удалось сразу сохранить пользователя %s: %s", user_id, exc)
...
user_profiles = _load_json_dict(USER_PROFILES_FILE)
user_profiles = {
    str(k): v for k, v in user_profiles.items()
    if str(k).lstrip("-").isdigit() and isinstance(v, dict)
}
user_profiles_dirty = False

def save_user_profile(message: Message):
    global user_profiles, user_profiles_dirty
    try:
        u = message.from_user
        uid = int(u.id)
        now = datetime.now().isoformat(timespec="seconds")
        p = user_profiles.setdefault(str(uid), {})
        p.update({
            "id": uid,
...
async def maintenance_loop():
    while True:
        try:
            ...
            flush_user_activity(force=False)
...
def _safe_filename(name: str, fallback="file"):
    name = os.path.basename(str(name or fallback))
    name = re.sub(r"[^0-9A-Za-zА-Яа-яЁё._-]+", "_", name).strip("._")
...
def get_quick_actions_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📁 Документы"), KeyboardButton(text="📄 Условия")],
...
def flush_persistent_state(force: bool = False):
    global users_db_dirty, stats_db_dirty, favorites_dirty, users_ids_dirty, modes_db_dirty, artifacts_dirty, user_profiles_dirty, last_persist_flush_at
    now = time.monotonic()
    ...
    if user_profiles_dirty:
        try:
            _atomic_json_write(USER_PROFILES_FILE, {str(k): v for k, v in user_profiles.items()})
            user_profiles_dirty = False
        except Exception as exc:
            logging.error("Не удалось сохранить профили пользователей: %s", exc)
...
_USER_ACTIVITY_CACHE = None
_USER_ACTIVITY_DIRTY = False

def _load_user_activity_store():
    global _USER_ACTIVITY_CACHE
    if _USER_ACTIVITY_CACHE is None:
        _USER_ACTIVITY_CACHE = _load_json_dict(USER_ACTIVITY_FILE)
    return _USER_ACTIVITY_CACHE

def _record_activity(user_id: int, kind: str, details: str = "", status: str = "ok"):
    global _USER_ACTIVITY_DIRTY
    try:
        data = _load_user_activity_store()
        uid = str(user_id)
        now = datetime.now().isoformat(timespec="seconds")
        item = data.setdefault(uid, {"events": [], "first_seen": now})
        item["last_seen"] = now
        item["events"].append({
            "ts": now,
            "kind": str(kind)[:50],
            "details": str(details or "")[:700],
            "status": str(status)[:20],
        })
        item["events"] = item["events"][-100:]
        _USER_ACTIVITY_DIRTY = True
    except Exception:
        pass

def flush_user_activity(force=False):
    global _USER_ACTIVITY_DIRTY
    if not _USER_ACTIVITY_DIRTY and not force:
        return
    try:
        _atomic_json_write(USER_ACTIVITY_FILE, _load_user_activity_store())
        _USER_ACTIVITY_DIRTY = False
    except Exception:
        pass
...
async def cb_ppt_wait_images(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Отправь одну или несколько фотографий. После этого нажми «🚀 Создать».\n"
...
async def handle_document(message: Message):
    _record_activity(message.from_user.id, "document", (message.caption or message.document.file_name or "Документ")[:700])
    save_user_profile(message)
...
async def handle_voice(message: Message):
    _record_activity(message.from_user.id, "voice", "Голосовое сообщение")
    save_user_profile(message)
    save_user_id(message.from_user.id)
    get_user_stats(message.from_user.id)
    """
...
async def handle_photo(message: Message):
    _record_activity(message.from_user.id, "image", (message.caption or "Изображение")[:700])
    save_user_profile(message)
    save_user_id(message.from_user.id)
    get_user_stats(message.from_user.id)
    user_id = message.from_user.id
...
async def handle_text(message: Message):
    _record_activity(message.from_user.id, "text", (message.text or "")[:700])
    save_user_profile(message)
    save_user_id(message.from_user.id)
    get_user_stats(message.from_user.id)
    user_id = message.from_user.id
    ...
    if user_id in ppt_states:
        ...
        if not current_topic:
            ...
            await message.answer(
                ...
                "</b>. Если картинки не нужны — "
                "используй кнопку <b>«🚀 Начать генерацию»</b>.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
def _admin_stats_report() -> str:
    total_messages = sum(int(v.get("messages", 0) or 0) for v in user_stats.values())
    total_voice = sum(int(v.get("voice", 0) or 0) for v in user_stats.values())
    total_files = sum(int(v.get("files", 0) or 0) for v in user_stats.values())
    total_images = sum(int(v.get("images", 0) or 0) for v in user_stats.values())
    total_exports = sum(int(v.get("exports", 0) or 0) for v in user_stats.values())
    return (
        "📊 <b>СТАТИСТИКА</b>\n\n"
        f"👥 Пользователей: <b>{len(all_users_cache)}</b>\n"
        ...
        f"⚙️ Сейчас занято: <b>{len(busy_users)}</b>\n"
        f"⏳ Очередь: <b>{HEAVY_WAITERS}</b>"
    )
...
async def admin_callback(call: CallbackQuery):
    ...
    if action == "dashboard":
        ...
        await call.message.edit_text(
            "🖥 <b>СЕРВЕР</b>\n\n"
            f"🐍 Python/PID: <code>{os.getpid()}</code>\n"
            f"💾 RAM процесса: <b>{ram:.0f} MB</b>\n"
            f"💿 Свободно на диске: <b>{f"{disk:.0f} MB" if disk is not None else "н/д"}</b>\n"
            f"⚙️ Активных задач: <b>{len(busy_users)}</b>\n"
            f"⏳ Тяжёлая очередь: <b>{HEAVY_WAITERS}</b>\n"
        ...
        ids = sorted(all_users_cache, reverse=True)
        per_page = 8
        page = 0
        pages = max(1, (len(ids) + per_page - 1) // per_page)
        lines = [f"👥 <b>ПОЛЬЗОВАТЕЛИ</b>", f"Всего: <b>{len(ids)}</b>", ""]
        for i, uid in enumerate(ids[:per_page], 1):
            p = profiles.get(str(uid), {})
            name = html.escape(str(p.get("first_name") or "Без имени"))
            username = html.escape("@" + p["username"]) if p.get("username") else "без username"
            last_seen = html.escape(str(p.get("last_seen") or "—"))
            lines.append(f"{i}. 👤 <b>{name}</b>\n   {username} · <code>{uid}</code> · 🕐 {last_seen}")
        nav = []
        if pages > 1:
            nav.append(InlineKeyboardButton(text="➡️", callback_data="adm_users_page:1"))
        ...
        reasons = []
        if ram >= RAM_HARD_LIMIT_MB:
            health = "🔴 КРИТИЧЕСКАЯ НАГРУЗКА"; reasons.append("RAM выше hard-limit")
        elif ram >= RAM_SOFT_LIMIT_MB:
            health = "🟡 ВЫСОКАЯ НАГРУЗКА"; reasons.append("RAM выше soft-limit")
        if disk is not None and disk < 250:
            health = "🔴 МАЛО ДИСКА"; reasons.append("свободно менее 250 MB")
        await call.message.edit_text(
            "🚨 <b>СОСТОЯНИЕ</b>\n\n"
            f"{health}\n"
            f"💾 RAM: <b>{ram:.0f} MB</b>\n"
            f"💿 Диск: <b>{f"{disk:.0f} MB" if disk is not None else "н/д"}</b>\n"
            f"⚙️ Активных задач: <b>{len(busy_users)}</b>\n"
            f"⏳ Очередь: <b>{HEAVY_WAITERS}</b>\n"
...
@dp.callback_query(F.data.startswith("adm_users_page:"))
async def admin_users_page_callback(call: CallbackQuery):
    ...
    nav.append(InlineKeyboardButton(text=f"{page+1}/{pages}", callback_data="adm:users"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"adm_users_page:{page+1}"))
    rows = [
        nav,
...
@dp.callback_query(F.data == "adm:activity")
async def admin_activity_callback(call: CallbackQuery):
    ...
    for uid, item in data.items():
        for e in (item.get("events") or [])[-20:]:
            rows.append((e.get("ts", ""), uid, e))
    rows.sort(reverse=True, key=lambda x: x[0])
    lines = ["📜 <b>АКТИВНОСТЬ ПОЛЬЗОВАТЕЛЕЙ</b>", ""]
    for ts, uid, e in rows[:40]:
        status = "✅" if e.get("status") == "ok" else "❌"
        lines.append(
            f"{status} <code>{html.escape(str(ts))}</code> · "
            f"<code>{html.escape(str(uid))}</code> · "
            f"<b>{html.escape(str(e.get('kind') or 'request'))}</b>\n"
            f"💬 {html.escape(str(e.get('details') or '—')[:220])}"
        )
    if len(lines) == 2:
...
async def main():
    ...
    print("🚀 Бот MecauAI 2.1 запущен: оптимизированные ресурсы, очередь, watchdog, артефакты и редактор активны...")
    try:
        await dp.start_polling(bot)
    finally:
        flush_persistent_state(force=True)
        flush_user_activity(force=True)
        maintenance.cancel()
        try:
            await maintenance
        except asyncio.CancelledError:
