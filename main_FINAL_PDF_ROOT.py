...
for _name in ("users.json", "favorites.json", "user_modes.json", "users_data.json", "user_stats.json", "artifacts.json", "admin_ids.json"):
...
def flush_persistent_state(force: bool = False):
    global users_db_dirty, stats_db_dirty, favorites_dirty, users_ids_dirty, modes_db_dirty, artifacts_dirty, last_persist_flush_at
    now = time.monotonic()
...
async def admin_callback(call: CallbackQuery):
    ...
    if action == "dashboard":
        ...
        await call.message.edit_text(f"👥 <b>ПОЛЬЗОВАТЕЛИ</b>\n\nВсего: <b>{len(load_user_ids())}</b>\nАктивных сейчас: <b>{len(busy_users)}</b>", parse_mode="HTML", reply_markup=_admin_back_keyboard())
    elif action == "audit":
        rows=[]
        for uid, events in ADMIN_AUDIT_LOG.items():
            for e in events[-5:]: rows.append(f"<code>{uid}</code> · {html.escape(e['ts'])} · {html.escape(e['action'])}")
        await call.message.edit_text("📋 <b>ИСТОРИЯ АДМИН-ДЕЙСТВИЙ</b>\n\n" + ("\n".join(rows[-30:]) or "Пока пусто."), parse_mode="HTML", reply_markup=_admin_back_keyboard())
    elif action == "admins":
